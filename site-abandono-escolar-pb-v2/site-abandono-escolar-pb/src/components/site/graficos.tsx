...
export function GraficoRede({
  dados,
}: {
  dados: { grupo: string; abandono: number; aprovacao: number; reprovacao: number; escolas: number }[];
}) {
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={dados} margin={{ left: 0, right: 8, top: 4, bottom: 4 }}>
        ...
        <Tooltip
          ...
          labelFormatter={(l: string) => `Rede ${l.toLowerCase()}`}
        />
        <Legend wrapperStyle={{ fontSize: 12 }} />
        <Bar name="Abandono" dataKey="abandono" fill="var(--danger)" radius={[6, 6, 0, 0]} maxBarSize={40} />
        <Bar name="Reprovação" dataKey="reprovacao" fill="var(--warn)" radius={[6, 6, 0, 0]} maxBarSize={40} />
...
export function GraficoInfraPorRede({ dados }: { dados: Record<string, string | number>[] }) {
  return (
    <ResponsiveContainer width="100%" height={Math.max(280, dados.length * 56)}>
      <BarChart data={dados} layout="vertical" margin={{ left: 8, right: 24, top: 4, bottom: 4 }}>
        <CartesianGrid horizontal={false} stroke="var(--border)" />
        <XAxis type="number" domain={[0, 100]} tick={eixo} tickLine={false} axisLine={false} unit="%" />
        <YAxis type="category" dataKey="item" width={130} tick={eixo} tickLine={false} axisLine={false} />
        <Tooltip {...tooltipProps} formatter={(v: number) => `${v}% das escolas`} />
        <Legend wrapperStyle={{ fontSize: 12 }} />
        <Bar dataKey="Estadual" fill="var(--chart-1)" radius={[0, 4, 4, 0]} barSize={12} />
        <Bar dataKey="Municipal" fill="var(--chart-2)" radius={[0, 4, 4, 0]} barSize={12} />
        <Bar dataKey="Privada" fill="var(--warn)" radius={[0, 4, 4, 0]} barSize={12} />
      </BarChart>
    </ResponsiveContainer>
  );
}
